...
- Firebase Auth stays for now. *(Changed in Round 3 — auth also moves off Firebase.)*
...
- Skip the heavy cutover work (dual-write, parallel run). Build it and switch.
  *(Round 3 goes further: there is no live data, so there is no data migration
  at all.)*
...
*(Named `last_talked` in this round. Renamed to `talk_events` in Round 4 — see
the naming rule there.)*
...
- **No migration script.** Do not write a Firestore → Postgres copy job. Create
  the Postgres schema and start empty. *(Round 5: the dummy-data seeding is
  dropped too, so a new account really is empty.)*
...
**`talk_events`, not `last_talked`.** A table name describes what *one row* is.
...
export const contacts = pgTable(
  ...
  (table) => [
    uniqueIndex("contacts_owner_name_unique").on(table.ownerId, sql`lower(${table.name})`),
...
**Not** Neon's *Managed* Better Auth, even though it is on the free tier.
...
free provider that sends to *arbitrary* recipients requires a **verified sending
...
1. **Nine files read `currentUser.uid` *and* `currentUser.email` together** to
...
- **Preview deployments get a new URL per deployment.** Google OAuth rejects
  callback URLs that are not registered. *Resolved by finding B:* assign a
